...
const COURIER_OPTIONS = [
  { id: 'india_post', name: 'India Post', desc: 'Government postal service · 5–7 days', icon: '📮' },
  { id: 'dtdc',       name: 'DTDC',       desc: 'Reliable courier · 3–5 days',          icon: '🚚' },
  { id: 'speed',      name: 'Speed',      desc: 'Fast delivery · 2–3 days',              icon: '⚡' },
  { id: 'safe',       name: 'Safe',       desc: 'Insured & secure · 3–5 days',           icon: '🛡️' },
];
...
const ComboOffers = () => {
  ...
  // Reset courier when state changes away from Kerala
  useEffect(() => {
    if (address.state !== 'Kerala') setSelectedCourier('');
  }, [address.state]);
  ...
  const goBackStep = () => {
    if (pendingProduct) { setPendingProduct(null); setSelectedSize(''); return; }
    if (currentStep > 0) {
      setCurrentStep(currentStep - 1);
      setSelectedSize('');
      setSearchQuery('');
  ...
  const handleAddressSubmit = (e) => {
    e.preventDefault();
    if (address.state === 'Kerala' && !selectedCourier) {
      alert('Please select a courier service for Kerala delivery');
      return;
    }
    setStage('payment');
  };
  ...
  const handlePayment = async (e) => {
    ...
    try {
      ...
      const orderItems = selections.map((sel) => ({
        ...structuredClone(sel.product), size: sel.size, quantity: 1,
      }));
      const options = {
        ...
        handler: async (response) => {
          const verifyRes = await fetch(`${backendUrl}/api/order`, {
            ...
            body: JSON.stringify({
              ...
              // Send courier only when Kerala
              courierService: address.state === 'Kerala' ? selectedCourier : null,
            }),
          });
  ...
  const isKerala = address.state === 'Kerala';
  ...
  return (
    <div className='border-t border-white pt-10 min-h-[80vh]'>
              ...
              {combos.map((combo) => (
                <div key={combo._id || combo.id} onClick={() => startCombo(combo)}
                    ...
                    {Array.from({ length: Math.min(combo.count, 5) }).map((_, i) => {
                      ...
                      return (
                        <div key={i}
                          ...
                          style={{ left: `${i * 22}px`, top: 0, transform: `rotate(${(i - (total - 1) / 2) * 5}deg)`, zIndex: i, height: '52px', width: '40px' }}>
                          {combo.image ? (
                  ...
                  <div className='flex items-center justify-between'>
                    {combo.discount > 0 ? (
                      <span className='bg-green-800 text-green-300 text-xs px-3 py-1 rounded-full font-semibold'>{combo.discount}% OFF</span>
                    ) : <span />}
              ...
              <React.Fragment key={idx}>
                <div className={`flex items-center gap-2 flex-shrink-0 ${idx <= currentStep ? 'text-green-400' : 'text-gray-500'}`}>
                  <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold border-2 ${
                    ...
                    idx === currentStep ? 'border-green-400 text-green-400' : 'border-gray-600 text-gray-600'}`}>
              ...
              {pendingProduct ? (
                <div>
                  <div className='flex items-center gap-4 mb-5'>
                    <img src={pendingProduct.image?.[0]} alt={pendingProduct.name} className='w-16 h-16 rounded-xl object-cover border border-green-800' />
                    <div>
                  ...
                  <div className='flex gap-2 flex-wrap mb-6'>
                    {(pendingProduct.sizes?.length > 0 ? pendingProduct.sizes : ['XS','S','M','L','XL','XXL']).map((sz) => (
                      <button key={sz} type='button' onClick={() => setSelectedSize(sz)}
                        className={`px-4 py-2 rounded border text-sm font-medium transition ${
                          selectedSize === sz ? 'bg-green-600 border-green-600 text-white' : 'border-gray-600 text-gray-300 hover:border-green-500'}`}>
                        {sz}
                      </button>
                    ))}
                  </div>
                  <button type='button' onClick={handleConfirmSize} disabled={!selectedSize}
                ...
                <>
                  <h3 className='text-white text-lg font-semibold mb-1'>Step {currentStep + 1}: {selectedCombo.slots[currentStep].label}</h3>
                  <p className='text-gray-400 text-sm mb-4'>Tap a product to choose its size</p>
                  <input type='text' placeholder={`Search ${selectedCombo.slots[currentStep].category}...`}
                      ...
                      {filteredProducts(selectedCombo.slots[currentStep].category).map((p) => (
                        <div key={p._id} onClick={() => handleTapProduct(p)}
                          ...
                          <div className='relative w-full overflow-hidden rounded-t-lg' style={{ height: '150px' }}>
                            <img src={p.image?.[0]} alt={p.name} className='w-full h-full object-cover object-top transition-transform duration-300 group-hover:scale-105' />
              ...
              {selections.length > 0 && (
                <div className='mt-4 p-3 border border-green-800 rounded-lg'>
                  ...
                  {selectedCombo.discount > 0 && (
                    <p className='text-green-400 text-xs'>Discount ({selectedCombo.discount}%): -{currency}{((parseFloat(subtotalAmount()) * selectedCombo.discount) / 100).toFixed(2)}</p>
                  )}
      ...
      {/* ── Address ── */}
      {stage === 'address' && (
        <form onSubmit={handleAddressSubmit} className='max-w-xl mx-auto'>
          ...
          <div className='flex flex-col gap-4'>
            ...
            {/* ── Courier Selection — only for Kerala ── */}
            {isKerala && (
              <div className='mt-2'>
                <p className='text-white text-sm font-semibold mb-1'>
                  Select Courier Service
                  <span className='text-green-400 text-xs font-normal ml-2'>(Required for Kerala)</span>
                </p>
                <p className='text-gray-400 text-xs mb-3'>Choose how you'd like your order delivered</p>
                  ...
                  {COURIER_OPTIONS.map((courier) => (
                    ...
                    >
                      <div className='flex items-center gap-2 mb-1'>
                        ...
                        <span className={`text-sm font-semibold ${selectedCourier === courier.id ? 'text-green-400' : 'text-white'}`}>
                          {courier.name}
                        </span>
                        {selectedCourier === courier.id && (
                          <span className='ml-auto w-4 h-4 rounded-full bg-green-500 flex items-center justify-center text-[10px] text-black font-bold flex-shrink-0'>✓</span>
                        )}
                      </div>
                      <p className='text-gray-500 text-[11px] leading-tight'>{courier.desc}</p>
                    </div>
                  ))}
                </div>
                {!selectedCourier && (
                  <p className='text-yellow-500 text-xs mt-2'>⚠ Please select a courier to continue</p>
                )}
              </div>
            )}
          </div>
          ...
          <div className='flex gap-3 mt-6'>
            ...
            <button type='submit' disabled={isKerala && !selectedCourier}
              className='flex-1 bg-green-700 text-white py-3 rounded hover:bg-green-600 transition text-sm font-semibold disabled:opacity-50 disabled:cursor-not-allowed'>
              Continue to Payment →
            </button>
          </div>
          {isKerala && !selectedCourier && (
            <p className='text-yellow-500 text-xs text-center mt-2'>Select a courier service above to continue</p>
          )}
        </form>
      )}
      ...
      {stage === 'payment' && (
        <form onSubmit={handlePayment} className='max-w-xl mx-auto'>
          ...
          {/* Show selected courier on payment page */}
          {isKerala && selectedCourier && (
            <div className='border border-green-800 rounded-xl p-3 bg-green-950 flex items-center gap-3 mb-4'>
              <span className='text-xl'>{COURIER_OPTIONS.find(c => c.id === selectedCourier)?.icon}</span>
              <div>
                <p className='text-green-400 text-xs font-semibold uppercase tracking-wider'>Courier</p>
                <p className='text-white text-sm font-bold'>{COURIER_OPTIONS.find(c => c.id === selectedCourier)?.name}</p>
                <p className='text-gray-500 text-[11px]'>{COURIER_OPTIONS.find(c => c.id === selectedCourier)?.desc}</p>
              </div>
            </div>
          )}
